...
func TestDashboardRendersHTMLForBrowsers(t *testing.T) {
	db := setupLinkAPITestDB(t)
	user := createLinkTestUser(t, db)
	link := createVersionedLink(t, db, user.ID, "browser-code", "https://example.com/browser", "initial")

	request := requestWithSession(t, map[string]any{"authenticated": true, "userid": user.ID})
	...
	for _, expected := range []string{"AtomicURL Dashboard", link.Code, link.Destination, "View history", "Create a link"} {
